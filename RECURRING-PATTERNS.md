...
דפוסים שאושרו ב**שני פרויקטי מקור שונים** — סיגנל חזק, אבל בסיס הראיות צר יותר מ-CORE. (הניסוח המקורי היה "2 מתוך 3", מהתקופה שבה היו שלושה מסמכי מקור; היום יש שישה, והבר המופעל הוא מה שהכלל התכוון אליו: שני פרויקטים שונים, לא יחס מספרי.) החל אם הפרויקט שלך תואם ל-stack. כל ערך מציין את הפרויקטים הספציפיים, כדי שתוכל לשפוט אם הדפוס סביר להיות רלוונטי.
...
- **"היום" לפי המכונה.** `date.today()` / `datetime.now()` בלי אזור זמן, על שרת שחי ב-UTC.
...
2. `datetime.now()` / `date.today()` בלי ארגומנט אזור זמן — בקוד שרת, בברירת מחדל של שדה, או בהשוואה מול ערך שהגיע מהמשתמש.
