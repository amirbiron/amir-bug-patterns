...
- **CodeBot (PRs #3436, #3437, #3440 — העותק בפרוזה):** אותה עובדה שהקוד מגדיר במקום אחד נוסחה מחדש ב-docstrings, בהערות ובעמודי תיעוד — ברירת המחדל של `max_sections` בשבעה מקומות, "הבלוב נטען לזיכרון לפני בדיקת הגודל" בארבעה, וגבול ה-front matter בשלושה. שלושתם תוקנו באותו יום, וכל תיקון קוד דרש לאתר ולערוך 4–7 קטעי פרוזה.
- **CodeBot (‏`tests/test_md_parser_oracle.py`) — כאן זה באמת נסחף:** ‏`services/md_parser.py` אמר 11,400 צורות, ‏`requirements/base.txt` אמר 11,507, והמחוללים ייצרו מספר שלישי, כי משפחת צורות נוספה בלי שאיש עדכן את המשפטים. אף אחד לא "נגע" במספר — ולכן רק טסט שגוזר ומשווה תופס את זה.
...
4. **והעותק שכתוב בפרוזה** — docstring, הערה, או עמוד תיעוד שמנסחים מחדש ערך שהקוד מגדיר במקום אחר: ברירת מחדל, קבוע, ספירה, קוד שגיאה, או מה רכיב אחר עושה. הוא אינו נראה כקוד, ולכן אף חיפוש אחרי פונקציה כפולה לא ימצא אותו. **מה שמבדיל עותק נסבל מממצא הוא אם הפרוזה נוקבת בשם המזהה של הבעלים** — שם אפשר למצוא ב-`grep`, ערך מוקלד לא. הנוסח המלא ב-`bugbot-rules/prose-restates-code-fact.md`.
...
- `bugbot-rules/prose-restates-code-fact.md` — העותק שכתוב בפרוזה (סעיף 4), עם התיקונים וה-False positives שלו
