...
3. כלל לפי שם חייב להתייחס לשם כמקטע: `auth_token`, `oauth_token`,
   ...
   התחילית עצמה חייבת להסתיים במפריד (`_`, `-`, `.`) **או בגבול
   camelCase**, אחרת `apiKey`, `accessToken`, `privateKey` ו-`clientSecret`
   מפוספסים — אותם שמות בכתיב אחר. הגבול מוגדר `[a-z0-9]` ← `[A-Z]`
   ו**הספרה בצד השמאלי נדרשת**: `v2Token`, `sha256Token` ו-`x509Key`
   מפוספסים בלעדיה. וגבול ה-camelCase חייב להיות רגיש-רישיות: בדפוס תחת
