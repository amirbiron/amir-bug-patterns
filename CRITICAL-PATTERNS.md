...
3. כלל לפי שם חייב להתייחס לשם כמקטע: `auth_token`, `oauth_token`,
   ...
   מפוספסים — אותם שמות בכתיב אחר. גבול ה-camelCase חייב להיות
   רגיש-רישיות: בדפוס תחת `(?i)`, `[A-Z]` תופס גם אותיות קטנות והגבול
   מתדרדר ל"בין כל שתי אותיות", מה שמחזיר את `?monkey=`.
