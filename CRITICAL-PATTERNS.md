...
שני מסלולי כשל, ושניהם מחזירים `None` ולא חריגה:

1. **מרוץ.** `server_info()` הוא סיבוב רשת. קורא מקביל שנכנס באמצעו רואה
   שומר מאותחל וערך ריק.
2. **הרעלה קבועה — החמורה.** כשל ב-`server_info()` משאיר את `client`
   מוצב על לקוח שבור. ה-`except` זורק כמצופה, אבל כל קריאה עתידית מדלגת
   על האתחול ומחזירה `None` **לתמיד**. אינה דורשת מקביליות בכלל.

### למה זה CRITICAL ולא סתם באג מקביליות
