...
7. **מגבלה גורפת שחלה גם על נתיבי בדיקת דופק.** ‏`/` ו-`/health`
   ...
   שנראית למגביל כמו תוקף. **וזה אינו רק קצב:** אותה
   מלכודת נמדדה על תקרת **גודל** — ‏`GET /healthz` עם `Content-Length`
   מזויף ענה 413, כי המידלוור לא בדק קודם אם המתודה נושאת גוף בכלל.
...
- `body-read-outside-cheap-reject.md` — הפטור לנתיב הדופק בהקשר של תקרת גוף בקשה: בודקים רק מתודות שנושאות גוף.
