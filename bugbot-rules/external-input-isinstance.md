...
8. **גודל, ולא צורה** — ערך מחרוזתי שהגיע מחוץ לתהליך ואין עליו תקרת אורך, **כשבאותה פונקציה קלט אחר כן מוגבל**. האסימטריה הזו היא החתימה, והיא נראית על המסך בשורות סמוכות: הפרמטרים המספריים של הפונקציה עוברים clamp לטווח, והפרמטר המחרוזתי עובר `strip()` ותו לא. מה שנעשה עם הערך במורד הזרם — סריקה על כל רכיב, רגקס, פרסור — קובע כמה זה עולה, אבל אינו מה שמדליק את הכלל; השורש הוא הגבול החסר. **וארבע דרישות על התקרה:** היא **סירוב ולא חיתוך** (‏`value[:N]` לפני מסנן אבטחה בודק מחרוזת אחרת מזו שהתבקשה — ראה `silent-truncation-at-sink.md`), עם **קוד שגיאה משלו** ולא מאוחד עם "ערך חסר" (‏`blanket-policy-silent-block.md`), **במספר שנגזר ממדידה** — הערך האמיתי הארוך ביותר **כפול שוליים**, בסדר הזה, ולא מספר עגול שנבחר — **ובהכרה שתקרה לשדה אינה תחליף לתקרת גוף בקשה ולהגבלת קצב**: היא חוסמת את ההגברה בכל בקשה בודדת, לא את מספר הבקשות, ולכן היא הקטנת משטח ולא סגירה של הציר. שלושתן שכבות נפרדות, ונוכחות של אחת אינה פוטרת מהאחרות. שים לב שהתיקון אינו "להחיל על המחרוזת את אותו טיפול": ‏clamp על מספר הוא תיקון, clamp על מחרוזת הוא אובדן תוכן.
