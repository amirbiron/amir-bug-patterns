...
- `body-read-outside-cheap-reject.md` — מידלוור האימות צריך לא רק לדחות לפני שה-route רץ, אלא גם להיות **בחוץ** ביחס לכל שכבה שצורכת משאב לכל בקשה. ב-Starlette `add_middleware` מוסיף בחוץ, ואימות של ספרייה עשוי לשבת בפנים על mount — ואז הסדר הפוך ממה שנראה.
