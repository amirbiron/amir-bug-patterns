...
10. **קריאה שנראית כמו אימות אינה בהכרח שולחת בקשה.** SDKs מחזירים אובייקטים עצלים: `Github.get_user()` מחזיר `AuthenticatedUser` עם `completed=False`, וה-`GET /user` יוצא רק בגישה לשדה. אישורים פסולים "מתחברים" בהצלחה וה-`except` לא רץ. גע בשדה במפורש, בשורה משלה עם הערה — אחרת היא תוסר כקוד מת. אמת מול מקור ה-SDK, לא לפי שם המתודה.
