...
11. **כשל בערך החזרה ≠ חריגה.** לפני עטיפת קריאה ב-try/except — בדוק את מסלול הכשל של הפונקציה: זורקת, או מחזירה `None`/`False`/`0`? אם מחזירה falsy — חובה `if not result:` לפני כל דיווח הצלחה. אסור `saved = True` קבוע אחרי קריאה, ואסור ✅ למשתמש בלי תנאי על הערך. בפונקציה חדשה: ערוץ כשל אחד (עדיף זריקה), מתועד ב-docstring.
