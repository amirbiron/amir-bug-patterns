...
9. לבדיקת **שייכות** על ערך חיצוני (`value in <frozenset>`): בדיקת טיפוס **לפני** בדיקת החברות. `in` על קבוצה או מילון מחשב `hash(value)`, וערך לא-hashable זורק `TypeError` במקום להחזיר `False` — כלומר הוולידציה קורסת לפני שהספיקה לומר מה לא תקין, והמשתמש מקבל 500 במקום 400. השורה נראית תמימה בדיוק מפני שהיא נראית כמו שאלה שמחזירה כן/לא.

10. ל**גודל** של ערך חיצוני, ולא רק לצורתו: תקרת אורך, באותה נקודה שבה כבר יושבות שאר בדיקות הגבול. תשעת הסעיפים שמעל שואלים כולם "האם הערך תקין"; אף אחד מהם אינו שואל "האם הוא גדול". **הסימן שרואים על המסך הוא אסימטריה בין הקלטים של אותה פונקציה** — הקלטים המספריים עוברים clamp, והקלט המחרוזתי לא עובר כלום. הסיבה שזה שורד סקירה: השינוי שמרחיב השוואה אחת לסריקה על כל הרכיבים נראה כמו הידוק אבטחה, ולכן הוא נסקר לפי נכונות בלבד. הלולאה אינה השורש — היא רק המקום שבו הגבול החסר נהיה יקר. ארבע דרישות על התקרה עצמה: **(א) סירוב, לא חיתוך.** `value[:N]` לפני מסנן אבטחה בודק מחרוזת אחרת מזו שהתבקשה — `silent-truncation-at-sink.md` §2. ואל תגזור את ההיתר מה-clamp שיושב בשורה ממול: למספר clamp הוא תיקון, למחרוזת הוא אובדן תוכן. **(ב) קוד שגיאה משלו** (`path_too_long`), ולא איחוד עם "ערך חסר" — סירוב שאינו נוקב בסיבתו שולח את הקורא לחפש את הטעות בשם הקובץ, וזה `blanket-policy-silent-block.md`. **(ג) המספר נגזר ממדידה** — הערך האמיתי הארוך ביותר כפול שוליים, ולא מספר עגול שנבחר. **(ד) תקרה לשדה היא הקטנת משטח ולא תחליף** לתקרת גוף בקשה ולהגבלת קצב: היא חוסמת את ההגברה בכל בקשה בודדת, לא את מספר הבקשות.
...
- `bugbot-rules/secret-in-url-query.md` §4 ("וללא quantifier מקונן") — אותה משפחה בדיוק, מהצד של הרגקס: עלות שגדלה עם קלט שמישהו אחר שולט באורכו
- `docs/source-projects/codebot-patterns.md`, "אישושים לדפוסים קיימים" — הראיות המלאות לסעיף 10, כולל המדידה
