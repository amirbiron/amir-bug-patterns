...
> 
> ‏U3 היה מתועד, והייתה לו שורה — `PyGithub / קריאות SDK חיצוני` — והיא לא נדלקה על ולידציה של ערך שהגיע מגוף JSON. הסיבה: השורה ממופתחת לפי מה שאתה **קורא לו**, והדפוס הוא על מאיפה ה**נתון** הגיע. רוב הדוגמאות של U3 אינן קריאות SDK בכלל. מי שכותב את הקוד מסתכל בטבלה, לא מוצא שורה, וממשיך.
> 
> ולכן גם: **השורה מצביעה על הכלל הכללי ישירות.** הפניה לקובץ סטאק שמצליב לכלל היא קפיצה שנייה, ואותה עושים רק אחרי שכבר החלטת שהמקרה שלך בצורת אותו סטאק — כלומר בדיוק ההחלטה שהטבלה אמורה לחסוך.
> 
> המבחן לפני שכותבים שורה: **האם מי שעומד לכתוב את הקוד יזהה את עצמו בניסוח הזה, לפני שהוא יודע מה הבאג?** אם לא — הניסוח לפי הקטגוריה, לא לפי המשימה.
