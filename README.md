...
| `claude-md-snippets/critical.md` | **להדביק את התוכן ל-`CLAUDE.md` של הפרויקט** (תמציתי, 14 כללים). |
...
- שילוב כמה ל-prompt אחד לסקירה ממוקדת (למשל סקירת אבטחה = כללי K1..K10 + K12 בידוד דיירים + K13 סוד במחרוזת נגזרת + K14 סוד בשורת שאילתה + K15 הסקה מסיגנל עקיף; K11 הוא שלמות-נתונים, לא אבטחה — צרף אותו לסקירות correctness).
...
├── CRITICAL-PATTERNS.md         # K1..K15 — חומרה גבוהה, החל בכל מקום, החל בכל מקום
...
│   ├── inferring-external-state-from-indirect-indicator.md  # CRITICAL
│   ├── sibling-flow-asymmetry.md
│   └── fix-followup-regression.md
