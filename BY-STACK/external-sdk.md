...
## דפוס 11 — פרמטר שאינו קיים ב-API: ה-`try` הוא קוד מת
...
הצורה הזו **תמיד נראית תקינה בסקירה**: יש טיפול בשגיאות, יש הודעה מסבירה, יש מסלול חלופי. אבל אם החתימה האמיתית היא `def explain(self)`, אז ה-`try` הוא קוד מת, ה-`except` הוא **המסלול היחיד שרץ אי פעם**, וההודעה שלו מתארת את ההפך ממה שקורה.
...
### כלל
לכל לקוח SDK שמחזיק אישורים עם תפוגה: מסלול `refresh` שנקרא על 401/`invalid_grant`, ניסיון אחד נוסף, ורק אז כשל. ולציין מה קורה כשגם הריענון נכשל — התנתקות מוצהרת עדיפה על לקוח שנשאר בזיכרון ומחזיר 401 לנצח.
