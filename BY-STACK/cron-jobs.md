...
## דפוס 10 — הרצה ראשונה בלי baseline שולחת את כל הבקלוג

ג'וב שמסנכרן "מה חדש מאז הפעם הקודמת" חייב להכריע מה קורה כשאין **פעם קודמת**. בלי הכרעה, ההרצה הראשונה מתייחסת לכל ההיסטוריה שהיא רואה כ"חדש".

**האירוע (CodeBot PR #139):** לא נקבע baseline כשאין `notifications_last`, ולכן הלולאה עברה על עשרת ה-PRים האחרונים וכולם נחשבו חדשים. המשתמש שהפעיל התראות קיבל **מיד מטח של עשר הודעות** על דברים שקרו מזמן.

### תיקון
בהרצה הראשונה: לקבוע את הסמן על "עכשיו" ולא לשלוח כלום (`baseline only`), או לשלוח לכל היותר פריט אחד עם ציון מפורש שזה הראשון. ההחלטה נכתבת בקוד ליד ה-`if not last_seen:`, לא נגזרת ממה שקרה להיות בברירת המחדל.

### כלל לזיהוי
`if not last_run` / `last_seen or <ברירת מחדל>` בג'וב ששולח הודעות, כותב, או משנה מצב חיצוני — ואחריו לולאה על תוצאות. השאלה: *מה קורה בהרצה הראשונה על חשבון ותיק?*

### Commits אמיתיים
- CodeBot PR #139 (אוגוסט 2025).
